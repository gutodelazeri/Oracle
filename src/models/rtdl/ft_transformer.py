...
class DLfttransformer(RegressionModel):
    ...
    def __init__(self, parameters_info, model_info):
        ...
        zero.set_randomness(self.seed)
        self.tf = evaluation.Normalization(self.args, self.seed)
        ...
        self.model = Transformer(d_numerical=self.tf.n_num_features, categories=self.tf.cat_values,
...
if __name__ == "__main__":
    m = RegressionModelSetup(model={}, model_parameters={"seed": 0,
                                                         "weights": "src/models/rtdl/output/acotsp-30K/ft_transformer/tuned/0.toml",
                                                         "scenario": "acotsp"}, preprocessing={})

    rn = DLfttransformer({}, m)
